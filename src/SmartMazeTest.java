...
//        smartMaze.setCurrentCellKey(args[0] + "," + args[1]);
        smartMaze.setCurrentCellKey("1,3");
        smartMaze.setCurrentCell(smartMaze.getMaze().get(smartMaze.getCurrentCellKey()));
    ...
    public void keyPressed(KeyEvent e) {
    	...
    	if (moreMoves()) {
        	...
        	drawMaze();
	    	if (!onIdealPath()) {
	    		String message = "Move number " + moveNumber + " put you on the incorrect path.\n";
	    		message = message + "Current cell: " + currentCell.getX() + "," +currentCell.getY() + "\n";
	    		message = message + "Correct cell: " + idealPath.get(moveNumber).getX() + "," + idealPath.get(moveNumber).getY();
	    		
	    		JOptionPane.showMessageDialog(null, message, "Maze Status", JOptionPane.PLAIN_MESSAGE);
	    		
	    	}
    	} else {
    		JOptionPane.showMessageDialog(null, "No more moves to validate", "Maze Status", JOptionPane.PLAIN_MESSAGE);
    ...
    public void drawMaze() {
        ...
        for (Cell cell: maze.values()) {
        	int x = cell.getX();
        	int y = cell.getY();
        	if (cell.hasSouthWall()) line(x, y, x + 1, y);
            if (cell.hasNorthWall()) line(x, y + 1, x + 1, y + 1);
            if (cell.hasWestWall())  line(x, y, x, y + 1);
            if (cell.hasEastWall())  line(x + 1, y, x + 1, y + 1);
        }
	...
	public boolean onIdealPath() {
		...
		if (currentCell.getX()==idealPath.get(moveNumber).getX() && currentCell.getY()==idealPath.get(moveNumber).getY()) {
			return true;
		} else {
			return false;
		}
